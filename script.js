const works=["00007fail.png","00002fail.png","00010fail.png","00004fail.png","00001fail.png","00009fail.png","00006fail.png","00011fail.png","00003fail.png","00008fail.png","00005fail.png"];
...
function cardMarkup(index,set){
  ...
  return `<figure class="workCard"><button class="imageWrap" type="button" data-work="${index}" ${set>0?'tabindex="-1" aria-hidden="true"':''} aria-label="Увеличить работу Аллы ${index+1}"><img src="${works[index]}" alt="Работа Аллы ${index+1}" ${set>0||index>2?'loading="lazy"':''} draggable="false"></button><figcaption><span>Алла / ${n}</span><span>Маникюр</span></figcaption></figure>`;
}

function buildWorks(){
  rail.innerHTML=[0,1,2].map(set=>`<div class="worksSet" ${set>0?'aria-hidden="true"':''}>${works.map((_,i)=>cardMarkup(i,set)).join('')}</div>`).join('');
  galleryGrid.innerHTML=works.map((src,i)=>`<button class="galleryTile" type="button" data-gallery-work="${i}" aria-label="Увеличить работу Аллы ${i+1}"><img src="${src}" alt="Работа Аллы ${i+1}" ${i>2?'loading="lazy"':''}><span>${String(i+1).padStart(2,'0')}</span></button>`).join('');
}
...
function showWork(index){selectedWork=(index+works.length)%works.length;lightboxImage.src=works[selectedWork];lightboxImage.alt=`Работа Аллы ${selectedWork+1} в увеличенном виде`;lightboxCount.textContent=`${String(selectedWork+1).padStart(2,'0')} / ${String(works.length).padStart(2,'0')}`;workLightbox.classList.add('open');workLightbox.setAttribute('aria-hidden','false');lockBody()}
